...
function GroupListPage() {
  ...
  // 그룹 데이터를 서버에서 가져오기 isPublic 수정완료
  const fetchGroups = useCallback(async () => {
    try {
      const response = await fetch(`/api/groups?page=${page}&pageSize=${pageSize}&sortBy=${sortBy}&keyword=${searchKeyword}&isPublic=${formData.isPublic ? 1 : 0}`);
  ...
  const handleSortChange = (e) => {
    ...
    switch (newSortBy) {
      case 'latest':
        ...
        break;
      case 'mostPosted':
        sortedGroups.sort((a, b) => b.memories - a.memories);
        break;
      case 'mostLiked':
        sortedGroups.sort((a, b) => b.likes - a.likes);
        break;
      case 'mostBadge':
        sortedGroups.sort((a, b) => b.badges - a.badges);
        break;
      default:
        break;
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (!formData.name || !formData.introduction || !formData.password || nameError) {
      ...
      if (selectedFile) {
        ...
        try {
          ...
          setErrorMessage('이미지 업로드에 실패하였습니다.');
        }
      }
      ...
      <div style={{ marginBottom: '20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div>
          <button
            ...
            className={formData.isPublic ? 'button-selected' : 'button-unselected'}
          >
            공개
          </button>
          <button
      ...
      <div style={{ marginTop: '20px' }}>
        {formData.isPublic ? (
          <div className="group-grid public-grid">
            {filteredGroups.filter(group => group.isPublic).map((group) => (
              <div
                key={group.id}
                className="group-box"
              ...
              >
                ...
                <div className="group-info">
                  <p className="group-date-status">
                    ...
                    <span className={group.isPublic ? 'status-public' : 'status-private'}>
                  ...
                  <div className="group-stats">
                    <div className="stat-item">
                      ...
                      <span className="stat-value">{group.badges}</span>
                    </div>
                    <div className="stat-item">
                      <span className="stat-label">추억</span>
                      <span className="stat-value">{group.memories}</span>
                    </div>
                    <div className="stat-item">
                      <span className="stat-label">그룹 공감</span>
                      <span className="stat-value">{group.likes}K</span>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div className="group-grid private-grid">
            {filteredGroups.filter(group => !group.isPublic).map((group) => (
              ...
              >
                <div className="group-info">
                  <p className="group-date-status">
                    {formatDateDifference(group.createdAt)} <span className="status-private">| 비공개</span>
                  </p>
                  <h3 className="group-name">{group.name}</h3>
                  <div className="group-stats">
                    <div className="stat-item">
          ...
          <div className="modal-content">
            ...
            <form onSubmit={handleSubmit}>
              <div className="form-group group-name-wrapper">
                <label htmlFor="groupName" className="group-name-label">그룹명</label>
                <input
                  type="text"
                  id="groupName"
                  name="name"
                ...
                />
                {nameError && <p className="error-message">특수문자는 !@#$%^_만 사용하실 수 있습니다.</p>}
              </div>
              <div className="form-group">
                <label htmlFor="image-upload" className="label-image-upload">대표 이미지</label>
                <div className='image-upload-container'>
                  <div className={`file-select-box ${selectedFile ? 'filled' : ''}`}>
              ...
              <div className="form-group">
                <label htmlFor="introduction" style={{ textAlign: 'left' }}>그룹 소개</label>
                <textarea
                  id="introduction"
                  name="introduction"
              ...
              <div className="form-group">
                ...
                <div style={{ display: 'flex', alignItems: 'center', marginTop: '0px' }}>
                  <span className="group-public-text" style={{ marginRight: '10px' }}>공개</span>
              ...
              <div className="form-group">
                <label htmlFor="password" style={{ textAlign: 'left' }}>비밀번호 생성</label>
                <input
                  type="password"
                  id="password"
                  name="password"
