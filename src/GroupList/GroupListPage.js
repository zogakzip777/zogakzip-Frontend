...
function GroupListPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          formData.isPublic ? "그룹 등록에 실패하였습니다." : "비공개 그룹 등록 실패"
        );
      }
      ...
      <div style={{ marginTop: "20px" }}>
        {groups.length === 0 ? (
          <div className='empty-state'>
            ...
            <p className="empty-state-message2">가장 먼저 그룹을 만들어보세요!</p>  
      ...
      {passwordModalOpen && (
        <div className="group-modal password-modal">
          <div className="group-modal-content">
            ...
            <p className="group-password-message">비공개 그룹에 접근하기 위해 권한 확인이 필요합니다.</p>
            <input
              type="password"
            ...
            <div className="password-modal-buttons">
              <button className = "password-confirm-button" onClick={handlePasswordSubmit}>제출하기</button>
              <button className = "password-cancel-button" onClick={() => setPasswordModalOpen(false)}>취소</button>
            </div>
          </div>
        </div>
      )}
      ...
      {successMessage && (
        <div className="group-modal">
          <div className="group-modal-content group-success-modal">
            ...
            <button className="group-confirm-button" onClick={handleCloseMessageModal}>확인</button>
          </div>
        </div>
      )}
      ...
      {errorMessage && (
        <div className="group-modal">
          <div className="group-modal-content group-error-modal">
            <div className="error-message-text">{formData.isPublic ? "그룹 만들기 실패" : "그룹 접근 실패"}</div>
